...
import {
  AdaptiveHeading,
  AdaptiveParagraph,
} from "../components/AdaptiveText.jsx";
import "../styles.css";
...
const TRANSACTION_PATHS = ["bank_transfer", "upi_payment", "qr_payment"];
...
export default function TransactionPage() {
  ...
  const completeTransactionAuto = () => {
    if (window.__metricsCollector) {
      const completedTxn =
        window.__metricsCollector.completeTransaction("auto");
      console.log(
        "[TransactionPage] Auto-completed transaction:",
        completedTxn,
      );
    }
  ...
  const completeTransactionUser = () => {
    if (window.__metricsCollector) {
      const completedTxn =
        window.__metricsCollector.completeTransaction("user");
      console.log(
        "[TransactionPage] User-completed transaction:",
        completedTxn,
      );
    }
  ...
  const getStepDisplay = () => {
    ...
    if (pathType === "bank_transfer") {
      const steps = ["Account", "IFSC", "Confirm", "OTP"];
      ...
      <div className="card">
        ...
        {pathType && (
          <>
            ...
            >
              💳 Path: {pathType.replace(/_/g, " ").toUpperCase()} | Step{" "}
              {getStepDisplay()}
            </div>
                      ...
                      peers.map((peer) => (
                        <div
                          ...
                          style={{
                            ...
                            fontWeight:
                              receiver === peer.email ? "bold" : "normal",
                          }}
                          onMouseEnter={(e) => {
              ...
              confirmData && (
                ...
                >
                  <div style={{ marginBottom: "8px" }}>
                    <strong>Amount:</strong> ₹{confirmData.amount}
                  </div>
                  <div style={{ marginBottom: "8px" }}>
                    <strong>Recipient:</strong> {confirmData.receiver}
                  </div>
                  <div>
            ...
            {pathType === "upi_payment" && currentStep === 1 && confirmData && (
              ...
              >
                ...
                <div>
                  <strong>UPI ID:</strong> {confirmData.upiId}
                </div>
              </div>
            )}
            ...
            {pathType === "qr_payment" && currentStep === 0 && (
              ...
              >
                ...
                <AdaptiveParagraph>
                  Click "Scan QR Code" to simulate QR scanning
                </AdaptiveParagraph>
              </div>
            )}
            ...
            <AdaptiveButton
              ...
              style={{
                ...
                cursor:
                  loading || success || transactionActive
                    ? "not-allowed"
                    : "pointer",
              }}
            >
            ...
            {transactionActive && (
              ...
              >
                ...
                >
                  Transaction will auto-complete in {10 - transactionTimer}s, or
        ...
        <AdaptiveParagraph style={{ marginTop: "15px", fontSize: "12px" }}>
          This is a demo transaction form using{" "}
          {pathType ? pathType.replace(/_/g, " ") : "a transaction"} path. No
