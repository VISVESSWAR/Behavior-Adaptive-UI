...
import { UIProvider } from "./adaptation/UIContext.jsx";
import { TaskProvider } from "./task/TaskContext.jsx";
...
window.diagnoseCollection = async function () {
  ...
  c.snapshots.slice(-3).forEach((s, i) => {
    ...
    console.log(
      `  [${idx}] ${new Date(s.timestamp).toLocaleTimeString()} - action:${s.finalAction}, reward:${s.taskReward}`
    );
  });
